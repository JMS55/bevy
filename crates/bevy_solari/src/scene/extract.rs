...
pub fn extract_raytracing_scene(
    ...
    for (render_entity, mesh, material, transform, previous_frame_transform) in &instances {
        ...
        commands.insert((mesh.clone(), material.clone(), *transform));
        if let Some(previous_frame_transform) = previous_frame_transform.cloned() {
            commands.insert(previous_frame_transform);
        }
    }
}

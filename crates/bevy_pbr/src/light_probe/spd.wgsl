...
@group(0) @binding(1) var mip_1: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
@group(0) @binding(2) var mip_2: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
@group(0) @binding(3) var mip_3: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
@group(0) @binding(4) var mip_4: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
@group(0) @binding(5) var mip_5: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
@group(0) @binding(6) var mip_6: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, read_write>;
@group(0) @binding(7) var mip_7: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
@group(0) @binding(8) var mip_8: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
@group(0) @binding(9) var mip_9: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
@group(0) @binding(10) var mip_10: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
@group(0) @binding(11) var mip_11: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
@group(0) @binding(12) var mip_12: texture_storage_2d_array<#{OUTPUT_TEXTURE_FORMAT}, write>;
...
fn spd_downsample_mip_2(x: u32, y: u32, workgroup_id: vec2u, local_invocation_index: u32, base_mip: u32, slice: u32, subgroup_invocation_id: u32) {
    ...
    if local_invocation_index % 4u == 0u {
        spd_store((workgroup_id * 8u) + vec2(x / 2u, y / 2u), v, mip, slice);
    ...
    if local_invocation_index < 64u {
        ...
        spd_store((workGroupID.xy * 8u) + vec2(x, y), v, mip, slice);
...
fn spd_downsample_mip_3(x: u32, y: u32, workgroup_id: vec2u, local_invocation_index: u32, base_mip: u32, slice: u32, subgroup_invocation_id: u32) {
    ...
    if local_invocation_index < 64u {
        ...
        v = spd_reduce_quad(v, subgroup_invocation_id);
        if local_invocation_index % 4u == 0u {
            spd_store((workgroup_id * 4u) + vec2(x / 2u, y / 2u), v, mip, slice);
            spd_store_intermediate(x * 2u + y / 2u, y * 2u, v);
        }
    }
#else
    if local_invocation_index < 16u {
        ...
        spd_store((workgroup_id * 4u) + vec2(x, y), v, mip, slice);
...
fn spd_downsample_mip_4(x: u32, y: u32, workgroup_id: vec2u, local_invocation_index: u32, base_mip: u32, slice: u32, subgroup_invocation_id: u32) {
    ...
    if local_invocation_index < 16u {
        ...
        v = spd_reduce_quad(v, subgroup_invocation_id);
        if local_invocation_index % 4u == 0u {
            spd_store((workgroup_id * 2u) + vec2(x / 2u, y / 2u), v, mip, slice);
    ...
    if local_invocation_index < 4u {
        ...
        spd_store((workgroup_id * 2u) + vec2(x, y), v, mip, slice);
...
fn spd_downsample_mip_5(x: u32, y: u32, workgroup_id: vec2u, local_invocation_index: u32, base_mip: u32, slice: u32, subgroup_invocation_id: u32) {
    ...
    if local_invocation_index < 4u {
        ...
        v = spd_reduce_quad(v, subgroup_invocation_id);
        if local_invocation_index % 4u == 0u {
            spd_store(workgroup_id, v, mip, slice);
        }
    }
#else
...
fn spd_reduce_load_source_image(uv: vec2u, slice: u32) -> vec4f {
    ...
    let result = textureSample(mip_0, sampler_linear_clamp, texture_coord, slice);
    return vec4(srgb_from_linear(result.x), srgb_from_linear(result.y), srgb_from_linear(result.z), result);
}

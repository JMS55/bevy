...
@group(1) @binding(12) var accumulation_texture: texture_storage_2d<rgba32float, read_write>;
...
fn spatial_and_shade(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    let spatial_pixel_id = get_neighbor_pixel_id(global_id.xy, &rng);
    ...
    let spatial_base_color = pow(unpack4x8unorm(gpixel.r).rgb, vec3(2.2));
    ...
    let merge_result = merge_reservoirs2(input_reservoir, world_position, world_normal, diffuse_brdf,
        spatial_reservoir, spatial_world_position, spatial_world_normal, spatial_diffuse_brdf, &rng);
...
fn merge_reservoirs2(
    ...
    let canonical_target_function_canonical_sample = reservoir_target_function(canonical_reservoir, world_position, world_normal, diffuse_brdf);
    var other_target_function_canonical_sample = reservoir_target_function(canonical_reservoir, other_world_position, other_world_normal, other_diffuse_brdf);

    let other_target_function_other_sample = reservoir_target_function(other_reservoir, other_world_position, other_world_normal, other_diffuse_brdf);
    var canonical_target_function_other_sample = reservoir_target_function(other_reservoir, world_position, world_normal, diffuse_brdf);

    if reservoir_valid(canonical_reservoir) {
        other_target_function_canonical_sample *= trace_light_visibility(canonical_reservoir.sample, other_world_position);
    }

    if reservoir_valid(other_reservoir) {
        canonical_target_function_other_sample *= trace_light_visibility(other_reservoir.sample, world_position);
    }

    let canonical_mis_weight = (canonical_reservoir.confidence_weight * canonical_target_function_canonical_sample.a) /
        ((canonical_reservoir.confidence_weight * canonical_target_function_canonical_sample.a) + (other_reservoir.confidence_weight * other_target_function_canonical_sample.a));
    let canonical_resampling_weight = max(0.0, canonical_mis_weight) * (canonical_target_function_canonical_sample.a * canonical_reservoir.unbiased_contribution_weight);

    let other_mis_weight = (other_reservoir.confidence_weight * other_target_function_other_sample.a) /
        ((other_reservoir.confidence_weight * other_target_function_other_sample.a) + (canonical_reservoir.confidence_weight * canonical_target_function_other_sample.a));
    let other_resampling_weight = max(0.0, other_mis_weight) * (canonical_target_function_other_sample.a * other_reservoir.unbiased_contribution_weight);
    ...
    if rand_f(rng) < other_resampling_weight / combined_reservoir.weight_sum {
        ...
        let inverse_target_function = select(0.0, 1.0 / canonical_target_function_other_sample.a, canonical_target_function_other_sample.a > 0.0);
        combined_reservoir.unbiased_contribution_weight = combined_reservoir.weight_sum * inverse_target_function;

        return ReservoirMergeResult(combined_reservoir, canonical_target_function_other_sample.rgb);
    } else {
        combined_reservoir.sample = canonical_reservoir.sample;

        let inverse_target_function = select(0.0, 1.0 / canonical_target_function_canonical_sample.a, canonical_target_function_canonical_sample.a > 0.0);
        combined_reservoir.unbiased_contribution_weight = combined_reservoir.weight_sum * inverse_target_function;

        return ReservoirMergeResult(combined_reservoir, canonical_target_function_canonical_sample.rgb);
    }
}

...
@group(0) @binding(6) var<storage> transforms: array<GpuTransform>;
@group(0) @binding(7) var<storage> previous_frame_transforms: array<GpuTransform>;
...
fn resolve_triangle_data_full(instance_id: u32, triangle_id: u32, barycentrics: vec3<f32>) -> ResolvedRayHitFull {
    ...
    let transform = affine3_to_square(transforms[instance_id].affine_transpose);
    let previous_frame_transform = affine3_to_square(previous_frame_transforms[instance_id].affine_transpose);

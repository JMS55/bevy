...
fn sample_di(@builtin(workgroup_id) workgroup_id: vec3<u32>, @builtin(global_invocation_id) active_cell_id: vec3<u32>) {
    ...
    var good_light_target_functions: array<f32, 8u> = array(0.0);
    ...
    } else if combined_reservoir.unbiased_contribution_weight > 0.0 {
        // TODO: Can do this while doing RIS over the 8 lights
        var worst_index = 8u;
        var worst_target_function = 3.402823466e38;
        for (var i = 0u; i < 8u; i += 1u) {
            let t = good_light_target_functions[i];
            if combined_reservoir.sample_target_function > t && t < worst_target_function {
                worst_index = i;

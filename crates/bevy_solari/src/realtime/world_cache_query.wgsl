...
struct WorldCacheLight {
    light_id: u32,
    inverse_pdf: f32,
}

fn query_world_cache_light(world_position_in: vec3<f32>, world_normal: vec3<f32>, view_position: vec3<f32>, ray_t: f32, cell_lifetime: u32, rng: ptr<function, u32>) -> WorldCacheLight {
    var world_position = world_position_in;
    var cell_size = get_cell_size(world_position, view_position);
    ...
    for (var i = 0u; i < WORLD_CACHE_MAX_SEARCH_STEPS; i++) {
        ...
        if existing_checksum == checksum {
            // Cache entry already exists - get radiance
            let i = rand_range_u(8u, rng);
            let light = world_cache_sampling_light_ids[key * 8u + i];
            let inverse_pdf = world_cache_sampling_weights[key * 8u + i];
            return WorldCacheLight(light, inverse_pdf);
        } else if existing_checksum == WORLD_CACHE_EMPTY_CELL {
            // Cell is empty - initialize it
            world_cache_geometry_data[key].world_position = world_position;
            world_cache_geometry_data[key].world_normal = world_normal;
            return WorldCacheLight(NULL_LIGHT_ID, 0.0);
        } else {
            // Collision - linear probe to next entry
            key += 1u;
        }

...
// lobe at all, which `metallic` handles.
//
// `specular_occlusion` must match whatever the caller applies to its specular
// lobe. Only the energy the specular layer actually reflects is unavailable to
// the diffuse layer, so a renderer that occludes its specular has to say so here
// or a low reflectance loses the energy from both. Pass 1.0 if the specular lobe
// is not occluded.
fn calculate_diffuse_color(
    ...
    let specular_layer_reflectance =
        lighting::specular_reflectance(F0_dielectric, F_ab) * specular_occlusion;

    return base_color * (1.0 - metallic) * (1.0 - specular_transmission) *
        (1.0 - diffuse_transmission) * (1.0 - specular_layer_reflectance);
}
...
fn apply_pbr_lighting(
    ...
    lighting_input.multiscatter_factor = multiscatter_factor;
#ifdef STANDARD_MATERIAL_CLEARCOAT
    ...
    transmissive_environment_light_input.multiscatter_factor =
        lighting::compute_multiscatter_factor(transmissive_F_ab);
#ifdef STANDARD_MATERIAL_CLEARCOAT
    // No clearcoat.

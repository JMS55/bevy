...
const CONFIDENCE_WEIGHT_CAP = 20.0 * f16(INITIAL_SAMPLES);
...
fn spatial_and_shade(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    var pixel_color = merge_result.selected_sample_radiance * combined_reservoir.unbiased_contribution_weight * f32(combined_reservoir.visibility);
...
fn generate_initial_reservoir(world_position: vec3<f32>, world_normal: vec3<f32>, diffuse_brdf: vec3<f32>, rng: ptr<function, u32>) -> Reservoir{
    ...
    if reservoir_valid(reservoir) {
        ...
        reservoir.visibility = f16(trace_light_visibility(reservoir.sample, world_position));
        reservoir.unbiased_contribution_weight *= f32(reservoir.visibility);
    }

    reservoir.confidence_weight = f16(INITIAL_SAMPLES);
    return reservoir;
}
...
fn load_spatial_reservoir(pixel_id: vec2<u32>, depth: f32, world_position: vec3<f32>, world_normal: vec3<f32>, rng: ptr<function, u32>) -> Reservoir {
    ...
    if reservoir_valid(spatial_reservoir) {
        spatial_reservoir.visibility = f16(trace_light_visibility(spatial_reservoir.sample, world_position));
    }
...
fn merge_reservoirs(
    ...
    let mis_weight_denominator = 1.0 / f32(canonical_reservoir.confidence_weight + other_reservoir.confidence_weight);

    let canonical_mis_weight = f32(canonical_reservoir.confidence_weight) * mis_weight_denominator;
    ...
    let other_mis_weight = f32(other_reservoir.confidence_weight) * mis_weight_denominator;
    ...
    combined_reservoir.visibility = f16(max(0.0, (f32(canonical_reservoir.visibility) * canonical_resampling_weight
        + f32(other_reservoir.visibility) * other_resampling_weight) / combined_reservoir_weight_sum));
